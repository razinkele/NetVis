...
if (!Array.from) {
    Array.from = (function () {
        ...
        return function from(arrayLike/*, mapFn, thisArg */) {
            ...
            // 10. Let len be ToLength(Get(items, "length")).
            var len = toLength(items.length);

            // 11. Let A be a new array created as if by the expression new Array(len) where Array is
            // the standard built-in constructor with that name and len is the value of len.
            var A = isCallable(C) ? Object(new C(len)) : new Array(len);

            // 12. Let k be 0.
            var k = 0;
            // 13. Repeat, while k < len… (also steps a - h)
            var kValue;
            while (k < len) {
                kValue = items[k];
                if (mapFn) {
            ...
            // 14. Let putStatus be Put(A, "length", len, true).
            A.length = len;
            // 15. Return A.
            return A;
        };
...
// https://developer.mozilla.org/en-US/docs/JavaScript/Reference/Global_Objects/Function/bind#Compatibility// for fix some RStudio viewer bug (Desktop / windows)
